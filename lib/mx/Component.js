...
mx.Component = function()
{
    ...
    me.getLogger = function(p_name)
    {
        var name = p_name;
        if (mx.isEmptyString(p_name))
        {
            name = me.getClassName();
        }
        var logger = log4js.getLogger(name);
        return logger;
    };
    
    me.getModuleLogger = function()
    {
        return me.getLogger(me.getModule().id);
    };
    ...
    me.unbind = function(p_eventType, p_function)
    {
        if (isEmpty(p_eventType) && isEmpty(p_function))
        {
            for ( var name in me)
            {
                if (notEmpty(me[name]) && me[name].constructor === mx.Event)
                {
                    me[name].clear();
                    me[name] = null;
                }
